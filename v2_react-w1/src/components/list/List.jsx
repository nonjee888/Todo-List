...
function List({setTodos, todos}) {                        //todos와 setTodos를 부모컴퍼넌트에게서 받았음
    ...
    const TobeDone = (selectedId) => {                    
        const newTodos = todos.map((todo)=> {
         if (todo.id === selectedId) {                 //id 비교 => 넘겨 받은 id 와 기존 todo list 에 있는 id 와 비교. id 가 같다면 => { ...todo } 를 이용하여 기존 객체를 복사. 그리고 { ...todo, isDone: !todo.isDone } 을 이용하여 기존 객체의 isDone 을 한번 뒤집어 토글해준 값으로 덮어씌움.(취소로 변경됨)
    ...
    return (
        <div class="list-container">
            ...
            <div class="list-wrapper">
                {todos.map((todo) => {
                    if (todo.isDone === false) {
                        return <Todo todo={todo} key={todo.id} setTodos={setTodos} onRemove={onRemove} TobeDone={TobeDone} />  //todo가 isDone이 false라면 Working에 렌더링 되도록 Todo에게 정보전달
                    }
                }
                )}
            </div>
            ...
            <div class="list-wrapper">
                {todos.map((todo) => {
                    if (todo.isDone === true) {
                        return <Todo todo={todo} key={todo.id} setTodos={setTodos} onRemove={onRemove} TobeDone={TobeDone} />  //todo가 isDone이 true라면 Done에 렌더링 되도록 Todo에게 정보전달
                    }
                }
                )}
            </div>
